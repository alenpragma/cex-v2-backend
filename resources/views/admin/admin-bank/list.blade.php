@extends('admin.master',['menu'=>'admin_bank', 'sub_menu'=>'admin_bank_list'])
    ...
    <div class="custom-breadcrumb">
        <div class="row">
            ...
            <div class="col-sm-7 text-right">
                <a class="add-btn theme-btn" href="{{route('adminBankAdd')}}">{{__('Add New')}}</a>
            </div>
        </div>
    </div>
    <!-- /breadcrumb -->
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <ul class=" d-flex activity-menu">
                                                    <li>
                                                        <a title="{{__('Edit')}}" href="{{ route("adminBankEdit",["id" => $value->id]) }}">
                                                            <i class="fa fa-pencil"></i>
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a title="{{__('Delete')}}" href="{{ route("adminBankDelete",["id" => $value->id]) }}">
...
@section('script')
    <script>
        ...
        function statusChange(bank_id) {
            $.ajax({
                type: "POST",
                url: "{{ route('adminBankStatusChange') }}",
                data: {
                    '_token': "{{ csrf_token() }}",
                    'bank_id': bank_id
                },
